...
var _ = FDescribe("External Destination API", func() {
	...
	Describe("listing destinations", func() {
		...
		listDestinations := func(version, queryString, expectedResponse string) {
			var expectedResponseJson destinationsResponse
			...
			var responseJson destinationsResponse
			err = json.Unmarshal(responseString, &responseJson)
			Expect(err).NotTo(HaveOccurred())
			Expect(responseJson.TotalDestinations).To(Equal(expectedResponseJson.TotalDestinations))
			Expect(responseJson.Destinations).To(ConsistOf(expectedResponseJson.Destinations))
		...
		v1Response := `{
			...
			"destinations": [
				{ "protocol": "tcp", "ips": [ {"start": "23.96.32.148", "end": "23.96.32.149" } ] },
				{ "protocol": "tcp", "ports": [{"start": 8080, "end": 8081}], "ips": [ {"start": "23.96.32.150", "end": "23.96.32.151" } ] },
				{ "protocol": "icmp", "icmp_type": 1, "icmp_code": 2, "ips": [ {"start": "23.96.32.150", "end": "23.96.32.151" } ] }
			]
		}`
